...
class Dataset(object):
    ...
    def load_annotations(self):
        with open(self.annot_path, "r") as f:
            txt = f.readlines()     #그냥 한방에 다읽어오는 코드
            if self.dataset_type == "converted_coco":
                ...
                annotations = []
                for line in txt:
                    image_path = line.strip()     #여기는 다르게 strip으로 들어가네??? 그럼 빈칸들을 다 지워준다는 것이고 
                    root, _ = os.path.splitext(image_path)
                    with open(root + ".txt") as fd:     #뭐야 어쨌든 txt파일 여네???
                        boxes = fd.readlines()
                        string = ""
                        for box in boxes:
                            box = box.strip()
        ...
        with tf.device("/cpu:0"):
            ...
            if self.batch_count < self.num_batchs:
                while num < self.batch_size:
                    ...
                    if np.all(label_sbbox):
                      batch_smaller_target=-1
                      return (
                          image,batch_smaller_target
                      )
    ...
    def parse_annotation(self, annotation):
        line = annotation.split() #문제는 ____ (19).png 처럼 띄어져 있는 것들이 문제/ 띄어쓰기 이후에는 bounding box들이 나와야하기 때문
        image_path = line[0]    #이미 line[0]에 png가 같이 들어가는경우 하나와 #line[1]에 png가 있는 경우 2
        if '.png' in line[1]:
    ...
    def preprocess_true_boxes(self, bboxes):        #bboxes는 요소가 5개가 있음
        label = [
            np.zeros(
                (
                    ...
                    self.train_output_sizes[i],
                    self.anchor_per_scale,
                    5 + self.num_classes,         #4차원의 행렬 생성 여기서부터 위로 차례대로 열 행 차원 ~
                )
            )
            for i in range(3)       #총 4차원이 3개
        ]

        bboxes_xywh = [np.zeros((self.max_bbox_per_scale, 4)) for _ in range(3)]    #2차원이 3개
        bbox_count = np.zeros((3,))       #1차원
        ...
        for bbox in bboxes:
            ...
            for i in range(3):
                ...
                if np.any(iou_mask):
                    ...
                    if xind > 52 or yind > 52 or xind <-52 or yind<-52:
                      #label_sbbox, label_mbbox, label_lbbox = (-1,-1,-1,-1)*3
                      #sbboxes, mbboxes, lbboxes = (-1,-1,-1,-1)*3
            ...
            if not exist_positive:
                ...
                if xind > 52 or yind > 52 or xind <-52 or yind<-52:
                  ...
                  #return label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes
                  return 0,0,0,0,0,0
                label[best_detect][yind, xind, best_anchor, :] = 0        #왠지 
