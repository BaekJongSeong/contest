...
def get_image_names(image_path):
  image_s=image_path.split('/')
  for i in range(len(image_s)):
    if '.png' in image_s[i]:
      global image_pathh
      image_pathh = '/content/drive/MyDrive/result/'+(image_s[i].split('.'))[0]+'.txt'
      break
...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    for i in range(num_boxes[0]):
       ...
       # c1, c2 = (coor[1], coor[0]), (coor[3], coor[2])
        change = {'car' : 'car' , 'two-wheeled vehicle' : 'bicycle' , 'pedestrian' : 'person'}
     #   cv2.rectangle(image, c1, c2, bbox_color, bbox_thick)
        if show_label:
            bbox_mess = '%s: %.2f' % (classes[class_ind], score)
            
            #print(change[classes[class_ind]], score, int(coor[1]),int(coor[0]),int(coor[3]),int(coor[2]))
            all=[change[classes[class_ind]],score, int(coor[1]),int(coor[0]),int(coor[3]),int(coor[2])]
            all2.append(all)
            #f=open(image_pathh, 'a')
    ...
    d=open(image_pathh, 'w')
    for i in range(len(all2)):
      d.write(' '.join(map(str,all2[i])))
      d.write('\n')
    d.close()
    return image
